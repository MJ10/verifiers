...
You will be given a description of the current observations, and a list of available actions to choose from at each step. 
...
class MiniHackEnv(MultiTurnEnv):
    """
    ...
    def env_response(self,
        ...
        try: 
            obsv, reward, done, truncated, info  = state['env'].step(action) # what to do with info and reward 
        except Exception as e:
            # execute NOP (wait) instead
            obsv, reward, done, truncated, info  = state['env'].step(".")
            error_msg = str(e)
...
if __name__ == "__main__":
    env_name = sys.argv[1] if len(sys.argv) > 1 else "MiniHack-Room-5x5-v0"
    vf_env = MiniHackEnv()